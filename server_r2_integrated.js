...
const checkRAGSystem = async () => {
  console.log('🔍 检查Cloudflare R2 RAG系统状态...');
  ...
  try {
    ...
    if (missingVars.length > 0) {
      console.warn('⚠️  警告: 缺少R2环境变量:', missingVars.join(', '));
      console.warn('请在.env文件中配置完整的R2设置');
      return false;
    }
    ...
    return new Promise((resolve) => {
      const testProcess = spawn('python3', ['test_r2_connection.py'], {
      ...
      testProcess.on('close', (code) => {
        if (code === 0 && output.includes('所有检查通过')) {
          console.log('✅ Cloudflare R2 RAG系统已就绪');
          resolve(true);
        } else {
          console.warn('⚠️  警告: R2连接测试失败');
          console.warn('请运行: python3 test_r2_connection.py 检查配置');
          resolve(false);
        }
      });
      
      testProcess.on('error', (error) => {
        console.warn('⚠️  警告: 无法运行R2连接测试:', error.message);
        resolve(false);
      });
    ...
    console.warn('⚠️  警告: 检查R2系统时出错:', error.message);
    return false;
  }
};
...
const initializeServer = async () => {
  ...
  // 检查R2 RAG系统
  try {
    ragSystemReady = await checkRAGSystem();
    if (ragSystemReady) {
      console.log('✅ Cloudflare R2 RAG系统初始化完成');
    } else {
      console.warn('⚠️  R2 RAG系统未就绪，将使用备用模式');
    }
  } catch (error) {
    console.error('❌ R2 RAG系统检查失败:', error.message);
...
const callRAGSystem = async (userInfo, imageInfos, enhancedQuery = null) => {
  console.log('🧠 正在调用R2版本RAG系统进行深度分析...');
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      const inputData = {
        query: actualQuery,
        context: `用户信息: 昵称=${userInfo.nickname}, 职业=${userInfo.profession}, 年龄=${userInfo.age}`,
        diagnostic_mode: process.env.RAG_DIAGNOSTIC_MODE === 'true' || false
      };
      
      const inputJson = JSON.stringify(inputData);
      console.log('📤 发送给R2 RAG系统的数据:');
      console.log('   基本信息字段数:', Object.keys(userInfo).length);
      ...
      console.log('   存储类型: cloudflare_r2_simple');
      
      // 调用R2版本Python RAG查询服务
      const pythonProcess = spawn('python3', ['rag_query_service_r2_simple.py'], {
        cwd: __dirname,
        stdio: ['pipe', 'pipe', 'pipe'],
        encoding: 'utf8'
      });
      ...
      pythonProcess.on('close', (code) => {
        console.log(`🐍 R2 RAG进程结束，退出码: ${code}`);
        ...
        if (code === 0) {
          try {
            ...
            // 调试：显示原始输出
            console.log('📥 R2 RAG原始输出 (前100字符):', cleanOutput.substring(0, 100));
            
            // 解析Python返回的JSON
            const result = JSON.parse(cleanOutput);
            
            if (!result.error) {
              console.log('✅ R2 RAG系统分析完成');
              ...
              const ragData = {
                rag_analysis: {
                  ...
                  knowledge_references: result.sources || [],
                  storage_type: 'cloudflare_r2_simple'
                }
              };
              
              console.log('📊 R2 RAG分析详情:');
              console.log('   状态:', ragData.rag_analysis.status);
              console.log('   检索到文档数:', ragData.rag_analysis.sources_count);
              console.log('   知识回答长度:', ragData.rag_analysis.knowledge_answer.length, '字符');
              console.log('   存储类型:', ragData.rag_analysis.storage_type);
              ...
              if (ragData.rag_analysis.knowledge_references && ragData.rag_analysis.knowledge_references.length > 0) {
                ...
                ragData.rag_analysis.knowledge_references.forEach((ref, idx) => {
                  console.log(`     ${idx + 1}. 评分: ${ref.similarity_score?.toFixed(3) || 'N/A'}, 来源: ${ref.file_name || 'unknown'}`);
                });
              }
              
              resolve(ragData);
            } else {
              console.warn('⚠️ R2 RAG系统返回错误:', result.error);
              resolve(generateFallbackReport());
            }
          } catch (parseError) {
            console.error('❌ 解析R2 RAG响应失败:', parseError.message);
            console.error('原始输出 (前500字符):', outputData.substring(0, 500));
            resolve(generateFallbackReport());
          }
        } else {
          console.error('❌ R2 RAG进程执行失败，退出码:', code);
          if (errorData) {
      ...
      pythonProcess.on('error', (error) => {
        console.error('❌ 启动R2 RAG进程失败:', error.message);
        resolve(generateFallbackReport());
      });
      
      // 设置超时处理
      setTimeout(() => {
        console.warn('⏰ R2 RAG查询超时，终止进程');
        pythonProcess.kill();
        resolve(generateFallbackReport());
      }, 120000); // 120秒超时（2分钟），R2访问可能比本地稍慢
      
    } catch (error) {
      console.error('❌ 调用R2 RAG系统时发生错误:', error.message);
      resolve(generateFallbackReport());
    }
  });
};
...
app.get('/api/rag-status', async (req, res) => {
  ...
  try {
    ...
    if (missingVars.length === 0) {
      try {
        ...
        const testResult = await new Promise((resolve) => {
          ...
          setTimeout(() => {
            testProcess.kill();
            resolve({ status: 'timeout', error: '连接测试超时' });
          }, 8000);
        });
        
        r2Status = testResult.status;
        if (testResult.error) {
          r2Error = testResult.error;
        }
      ...
      r2Error = `缺少环境变量: ${missingVars.join(', ')}`;
    }
    ...
    res.json({
      ...
      r2_status: r2Status,
      r2_error: r2Error,
      bucket_name: process.env.BUCKET_NAME || 'not_configured',
      environment_variables: envStatus,
      missing_variables: missingVars,
      index_files: indexFiles,
      instructions: r2Status === 'connected' 
        ? 'R2 RAG系统已就绪，所有知识库查询将从云端加载'
        : 'R2系统未就绪，请检查配置或运行: python3 test_r2_connection.py',
      timestamp: new Date().toISOString()
    });
    
  } catch (error) {
    console.error('❌ 检查R2状态时出错:', error.message);
    res.status(500).json({
      rag_system_ready: false,
      storage_type: 'cloudflare_r2_simple',
      r2_status: 'error',
      r2_error: error.message,
      timestamp: new Date().toISOString()
    });
  }
});
...
const callPostDateRAGSystem = async (userQuestion, conversationHistory = []) => {
  ...
  try {
    ...
    console.log('📤 发送给R2 RAG系统的查询:');
    console.log('   用户问题长度:', userQuestion.length);
    console.log('   对话历史长度:', conversationHistory.length);
    console.log('   查询类型: post_date_debrief_r2');
    console.log('   存储类型: cloudflare_r2_simple');
    
    // 调用R2版本Python RAG系统
    const ragProcess = spawn('python3', ['rag_query_service_r2_simple.py'], {
      stdio: ['pipe', 'pipe', 'pipe']
    });
    
    // 发送输入数据
    ragProcess.stdin.write(JSON.stringify(ragInputData));
    ragProcess.stdin.end();
    
    // 设置超时
    const timeout = setTimeout(() => {
      ragProcess.kill();
      console.error('⏰ 情感教练R2 RAG查询超时（120秒）');
    }, 120000); // 120秒超时
    ...
    return new Promise((resolve, reject) => {
      ...
      ragProcess.on('close', (code) => {
        ...
        if (process.env.RAG_DIAGNOSTIC_MODE === 'true' && stderr.trim()) {
          console.log('🔬 R2 RAG诊断信息:');
          console.log(stderr);
        }
        
        if (code !== 0) {
          console.error('R2 RAG进程错误输出:', stderr);
          reject(new Error(`R2 RAG进程异常退出: ${code}`));
          return;
        }
        
        try {
          console.log('📥 R2 RAG原始输出 (前100字符):', stdout.substring(0, 100));
          const result = JSON.parse(stdout);
          
          console.log('✅ 情感教练R2 RAG分析完成');
          console.log('📊 R2 RAG分析详情:');
          console.log('   状态:', result.error ? 'error' : 'active');
          console.log('   存储类型:', result.storage_type || 'cloudflare_r2_simple');
          
          if (!result.error) {
            console.log('   检索到文档数:', result.sources_count || 0);
            console.log('   知识回答长度:', (result.answer || '').length);
            
            if (result.sources && result.sources.length > 0) {
              console.log('   引用文档:');
              result.sources.forEach((source, index) => {
                console.log(`     ${index + 1}. 评分: ${source.similarity_score?.toFixed(3) || 'N/A'}, 来源: ${source.file_name || 'unknown'}`);
              });
            }
          ...
          console.error('❌ 解析R2 RAG结果失败:', parseError.message);
          reject(parseError);
        }
      });
      ...
      ragProcess.on('error', (error) => {
        clearTimeout(timeout);
        console.error('❌ R2 RAG进程启动失败:', error.message);
        reject(error);
      });
    });
    
  } catch (error) {
    console.error('❌ 情感教练R2 RAG检索失败:', error.message);
    throw error;
  }
};
...
const callPostDateRAGSystemWithEnhancedQuery = async (enhancedQuery, originalUserQuestion, conversationHistory = []) => {
  console.log('🧠 开始情感教练R2 RAG检索（使用AI优化查询）...');
  
  try {
    // 构建适配R2 RAG系统的数据格式，使用增强查询
    const ragInputData = {
      query: enhancedQuery, // 使用AI优化后的查询
      context: `约会后复盘分析咨询 - 原始问题: ${originalUserQuestion.substring(0, 100)}`,
      diagnostic_mode: process.env.RAG_DIAGNOSTIC_MODE === 'true' || false
    };
    
    console.log('📤 发送给R2 RAG系统的优化查询:');
    ...
    console.log('   对话历史长度:', conversationHistory.length);
    console.log('   查询类型: post_date_debrief_enhanced_r2');
    console.log('   存储类型: cloudflare_r2_simple');
    ...
    return new Promise((resolve, reject) => {
      ...
      ragProcess.on('close', (code) => {
        ...
        try {
          ...
          console.log('✅ 情感教练R2 RAG分析完成（使用AI优化查询）');
          console.log('📊 R2 RAG分析详情:');
          console.log('   状态:', result.error ? 'error' : 'active');
          console.log('   存储类型:', result.storage_type || 'cloudflare_r2_simple');
